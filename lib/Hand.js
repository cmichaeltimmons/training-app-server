...
export default class Hand {
  constructor(cards) {
    ...
    */
    ...
    this.ranks = strength.ranks; // Set serialized cards.
...
function getHandStrength(hand) {
  ...
  */
  ...
  if ('4' in histogram) {
    ...
    const hasStraight = hand[0].rank - hand[4].rank == 4 || _.isEqual(hand.map(card => card.rank), [14, 5, 4, 3, 2]);

    if (hasFlush && hasStraight) {
      return {
        strength: c.HAND_STR_FLUSH,
        ranks: [histogram['1']]
      };
    } else if (hasFlush) {
      return {
        strength: c.HAND_FLUSH,
        ranks: [histogram['1']]
      };
    } else if (hasStraight) {
      return {
        strength: c.HAND_STRAIGHT,
        ranks: [histogram['1']]
      };
    } else {
      // High card.
      return {
        strength: c.HAND_HIGH,
        ranks: [histogram['1']]
      };
    }
  }
